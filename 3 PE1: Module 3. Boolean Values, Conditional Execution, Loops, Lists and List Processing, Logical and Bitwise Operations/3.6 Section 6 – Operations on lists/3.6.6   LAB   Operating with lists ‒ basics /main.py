...
def removeDuplicates(lsToRemove):
    operationOnList = lsToRemove[:]

    for num in range(len(lsToRemove)):
        print(operationOnList[num])

    return operationOnList

removedDuplicatesList = removeDuplicates(my_list)
print(f"List with removed duplicates: {removedDuplicatesList}")
